...
import { countryReport } from '@/bloc/services/analyticsService'
...
const statisticsChartConfig = computed(() => chartConfig(vuetifyTheme.current.value, countryLabels.value))
...
const getCountryReport = async () => {
	try {
		const response = await countryReport()
		...
		countryLabels.value = formattedResponse.map(it=> it.name)
	} catch (error) {
		console.log(error)
	}
}
